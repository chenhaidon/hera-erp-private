    ...
    LOOP
      IF (op.value->>'category') = 'outsourcing' THEN
        IF (op.value->'reports') IS NOT NULL AND jsonb_array_length(op.value->'reports') > 0 THEN
          ...
          SELECT id, factory_name INTO factory_id, factory_name
          FROM outsource_factories
          WHERE status = 'enabled'
            AND processing_capability = CASE op.value->>'name'
              WHEN '电脑绣' THEN '电脑绣花'
              WHEN '水洗' THEN '水洗整烫'
              ELSE processing_capability
            END
          ORDER BY created_at
          LIMIT 1;

          IF factory_id IS NULL THEN
            SELECT id, factory_name INTO factory_id, factory_name FROM outsource_factories WHERE status='enabled' ORDER BY created_at LIMIT 1;
          END IF;
          ...
          VALUES (ship_id, ship_no, rec.data->>'id', rec.data->>'work_no', op.value->>'code', op.value->>'name', rec.data->>'product_code', rec.data->>'product_name', factory_id, factory_name, ship_date, qty, '', '', 'shipped', now(), now());
          ...
          VALUES (ret_id, ret_no, ship_id, ship_no, rec.data->>'id', rec.data->>'work_no', op.value->>'code', op.value->>'name', rec.data->>'product_code', rec.data->>'product_name', factory_id, factory_name, ret_date, 'semi_finished', qty, qty, 0, 'qualified', 'stored', now(), now());
