...
BEGIN
  ...
  LOOP
    ...
    FOR i IN 0 .. jsonb_array_length(ops)-1 LOOP
      ...
      IF n > 0 THEN
        -- 按原时间排序
        SELECT array_agg(r ORDER BY (r->>'report_time')::timestamptz) INTO sorted_reports
        FROM jsonb_array_elements(reports) AS r;

        -- 按工序序号分配时间窗口，全部落在 8/30 前
        CASE seq
        ...
        FOR i IN 0 .. n-1 LOOP
          ...
          r := sorted_reports[i+1];
          r := jsonb_set(r, '{report_time}', to_jsonb(to_char(new_time AT TIME ZONE 'Asia/Shanghai', 'YYYY-MM-DD"T"HH24:MI:SS+08:00')));
          new_reports := new_reports || jsonb_build_array(r);
        END LOOP;
