      ...
      SELECT gen_random_uuid()::text, 'shipments',
        jsonb_build_object(
          ...
          'delivery_records', jsonb_build_array(
            jsonb_build_object('status','shipped','time', p_ship_date || ' 10:00:00','remark','已发货'),
            jsonb_build_object('status','signed','time', p_sign_date || ' 14:30:00','remark','客户已签收')
          ),
          'items', (SELECT data->'items' FROM entity_store WHERE entity_type='sales_orders' AND data->>'order_no'=p_order_no LIMIT 1),
          'boxes', (SELECT jsonb_agg(jsonb_build_object('box_no','CTN-'||LPAD(row_number() OVER (),3,'0'), 'product_code', item->>'product_code', 'quantity', item->>'quantity', 'gross_weight', round((item->>'quantity')::numeric*0.7,2), 'net_weight', round((item->>'quantity')::numeric*0.65,2), 'volume', round((item->>'quantity')::numeric*0.02,2)::text)) FROM entity_store, jsonb_array_elements(data->'items') AS item WHERE entity_type='sales_orders' AND data->>'order_no'=p_order_no),
          'created_at', v_now
        )
...
DROP FUNCTION IF EXISTS compress_contract_reports(text, timestamptz);
CREATE OR REPLACE FUNCTION compress_contract_reports(p_contract_no text, p_target timestamptz)
RETURNS TABLE(work_no text, old_max timestamptz, new_max timestamptz)
LANGUAGE plpgsql AS $$
DECLARE
...
DROP FUNCTION IF EXISTS set_few_defects(text);
CREATE OR REPLACE FUNCTION set_few_defects(p_contract_no text)
