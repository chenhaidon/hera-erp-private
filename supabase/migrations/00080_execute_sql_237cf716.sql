DROP FUNCTION IF EXISTS complete_26jlkxd006(); CREATE OR REPLACE FUNCTION complete_26jlkxd006() RETURNS TABLE (
  work_no text,
  action text,
  detail text
) LANGUAGE plpgsql AS $$
  ...
  LOOP
    ...
    FOR v_op_idx IN 0 .. jsonb_array_length(v_ops) - 1 LOOP
      ...
      IF v_j = 0 THEN
        ...
        v_pi_code := 'PI-' || split_part(v_work_no, '-', 3) || '-' || lpad((floor(random() * 900 + 100)::int)::text, 3, '0');
      ...
      IF v_op_category = 'outsourcing' THEN
        ...
        IF v_shipment_date IS NOT NULL THEN
          ...
          INSERT INTO outsource_shipments (
            ...
            gen_random_uuid(),
            'OS-' || replace(v_work_no, '-', '') || '-' || v_op_code,
            v_work_id,
            v_work_no,
            v_op_code,
            v_op_name,
            ...
            v_plan_qty,
            '安能物流',
            'LOG-' || replace(v_work_no, '-', '') || '-' || v_op_code,
            'shipped',
            v_now,
            v_now
          );
          ...
          INSERT INTO outsource_returns (
            id, return_no, shipment_no, work_order_id, work_order_no, operation_code, operation_name,
            ...
            gen_random_uuid(),
            'OR-' || replace(v_work_no, '-', '') || '-' || v_op_code,
            'OS-' || replace(v_work_no, '-', '') || '-' || v_op_code,
            v_work_id,
            v_work_no,
            v_op_code,
          ...
          INSERT INTO outsource_processing_payments (
            ...
            gen_random_uuid(),
            'OP-' || replace(v_work_no, '-', '') || '-' || v_op_code,
            gen_random_uuid(),
            v_work_no,
            v_op_code,
            v_op_name,
