...
export function MobileApprovals({ onBack }: { onBack?: () => void }) {
  return (
    <div className="flex h-full flex-col bg-background">
      <div className="border-b border-border bg-background px-4 pb-3 pt-6">
        <p className="text-xl font-bold text-foreground">审批中心</p>
        <p className="mt-1 text-sm text-muted-foreground">移动端审批中心入口</p>
      </div>
      <div className="flex flex-1 flex-col items-center justify-center p-4 pb-6 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">待办审批、已办审批、发起审批等功能即将上线</p>
        {onBack && (
          <button
